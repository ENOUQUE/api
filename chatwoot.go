...
// Corrigido para aceitar EXATAMENTE os 3 argumentos que o wmiau.go espera
func sendToChatwoot(postmap map[string]interface{}, userID string, db *sqlx.DB) {
	...
	if err != nil {
		log.Error().Err(err).Str("userID", userID).Msg("Erro ao buscar config do Chatwoot no banco")
		return
	}

	if !config.Enabled || config.URL == "" || config.Token == "" || config.InboxID == "" {
		return
	}
	...
	// 2. Cria ou Atualiza o Contato no Chatwoot
	...
	if err != nil || (resp.StatusCode() != http.StatusOK && resp.StatusCode() != http.StatusCreated) {
		...
		log.Error().Err(err).Int("status", status).Msg("Erro ao enviar mensagem para Chatwoot")
	}
}

// createChatwootContact garante que o contato exista
...
func handleChatwootWebhook(w http.ResponseWriter, r *http.Request, db *sqlx.DB, instanceName string) {
	...
	// Filtro: Apenas mensagens enviadas por agentes (evita loop infinito)
	if msgType != "outgoing" {
	...
	if myClient == nil || myClient.WAClient == nil {
		log.Error().Str("userID", userID).Msg("Cliente WhatsApp offline para envio de resposta Chatwoot")
...
func createOrGetChatwootInbox(config ChatwootConfig) (string, error) {
	...
	if err != nil || result.ID == 0 {
		return "", fmt.Errorf("não foi possível capturar o ID do inbox: %s", resp.String())
	}
...
func configureChatwootWebhook(config ChatwootConfig, webhookURL string) error {
	...
	if resp.StatusCode() != http.StatusOK {
		return fmt.Errorf("erro ao configurar webhook: status %d", resp.StatusCode())
	}
