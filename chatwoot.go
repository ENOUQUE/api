...
func createOrGetChatwootInbox(config ChatwootConfig) (string, error) {
	...
	chatwootURL := strings.TrimSuffix(config.URL, "/")package main
...
func sendToChatwoot(postmap map[string]interface{}, userID string, db *sqlx.DB) {
	...
	if contactIdentifier == "" {
		log.Warn().Str("sourceID", sourceID).Msg("Não foi possível validar contato no Chatwoot, tentando enviar mensagem assim mesmo...")
...
// createChatwootContact garante que o contato exista antes da mensagem ser enviada
func createChatwootContact(config ChatwootConfig, phone string, name string) string {
	...
	// Se retornar 200 ou 201, o contato foi criado/identificado
...
func transformEventToChatwoot(postmap map[string]interface{}, sourceID string) (map[string]interface{}, error) {
	rawEvt := postmap["event"].(*events.Message)
...
func handleChatwootWebhook(w http.ResponseWriter, r *http.Request, db *sqlx.DB, instanceName string) {
	var userID string
	if err := db.Get(&userID, "SELECT id FROM users WHERE name=$1", instanceName); err != nil {
